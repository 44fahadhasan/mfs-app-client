...
const CashOutPage = () => {
  ...
  const cashOutToAgent = (cashOutMonyData) => {
    axiosSecure
      .post("/cash-out", cashOutMonyData)
      .then(({ data }) => {
  ...
  const onSubmit = (data) => {
    ...
    const cashOutAmount = data.cashOutAmount;
    const pin = data.pin;

    const cashOutMonyData = {
      identifier,
      cashOutAmount,
      pin,
      email: user?.email,
    ...
    cashOutToAgent(cashOutMonyData);

    resetField("numberOrEmail");
    resetField("cashOutAmount");
    resetField("pin");
  };
      ...
      <div className="text-base-content font-medium  w-full overflow-hidden ">
        <div className="w-full mx-auto md:w-1/2 py-10 px-5 md:px-10">
          <div className="text-center mb-10">
            ...
            <SectionContent
              title={"Cash Out"}
              topContent={"1.5% fee of total balance"}
            />
          </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-1  text-base">
                ...
                {errors.cashOutAmount && (
                  <span className="text-base-content text-xs">
                    Cash Out amount can&apos;t be empty
                  </span>
                )}
              </div>
